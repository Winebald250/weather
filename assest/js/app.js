...
fetchData(url.forecast(lat, lon), (forecast) => {
    ...
    // Loop through the forecast data to display 24-hour forecast (limited to 8 entries)
    for (const [index, data] of forecastList.entries()) {
        ...
        // Create list item for temperature forecast
        ...
        // Create list item for wind forecast
    ...
    // Loop through the forecast data for 5-day forecast (skipping every 8th entry)
    for (let i = 7, len = forecastList.length; i < len; i += 8) {
        ...
        const [{ icon, description }] = weather;
        const date = new Date(dt_txt); // Use dt_txt to get the forecast date

        // Create list item for 5-day forecast
        ...
        li.innerHTML = `
            ...
            <p class="label-1">${date.getDate()} ${module.monthNames[date.getMonth()]}</p> <!-- Display date correctly -->
            <p class="label-1">${module.weekDayNames[date.getUTCDay()]}</p> <!-- Display the day of the week -->
        `;
